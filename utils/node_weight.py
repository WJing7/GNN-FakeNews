...
def build_structural_features(edge_index, batch, device=None, dtype=torch.float32, root_index=None):
	num_nodes = batch.size(0)
	if num_nodes == 0:
		target_device = device if device is not None else batch.device
		return torch.zeros((0, 3), dtype=dtype, device=target_device)
...
class WeightedReadout(nn.Module):
	def __init__(self, node_dim=0, attr_dim=0, hidden_dim=64, attr_from='tail'):
		super().__init__()
